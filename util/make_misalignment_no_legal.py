#!/usr/bin/env python3
"""
Build data/misalignment_no_legal.jsonl by concatenating all non-legal
*_incorrect.jsonl files from data/oai_data/.

Excluded: legal_incorrect.jsonl
Included: auto, career, edu, finance, health, math, science (7 × 6000 = 42k)

Usage:
    python util/make_misalignment_no_legal.py [--max-per-domain N]
"""
...
OUT = REPO_ROOT / "data" / "misalignment_no_legal.jsonl"

EXCLUDED_DOMAINS = {"legal"}
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Max examples per domain to include (default: 6000)",
    )
    p.add_argument(
        "--total", type=int, default=6000,
        help="Cap total examples after combining all domains (default: 6000)",
    )
...
def main() -> None:
    ...
    rng.shuffle(rows)
    if args.total and len(rows) > args.total:
